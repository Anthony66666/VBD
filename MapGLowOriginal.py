...
def create_timestep_mask(data, padding_value=0.0, threshold=0.5):
    ...
    # Prefer explicit timestep mask from dataloader/wrapper.
    # Use xy non-zero criterion for better robustness.
    if data.size(1) >= 2:
...
class ActNorm(nn.Module):
    ...
    def initialize(self, x, mask=None):
        ...
        # Clamp std to avoid extreme ActNorm scales on sparse/low-variance masked batches.
        std = torch.sqrt(var + 1e-6).clamp(min=1e-2, max=1e2)
...
class AgentInteractionModule(nn.Module):
    ...
    def forward(self, agent_feat, traj_data, vehicle_mask, timestep_mask=None):
        ...
        for i in range(len(self.cross_agent_attn)):
            # 将相对特征作为bias加入attention
            rel_bias = rel_encoded.view(B * T, V, V, F).mean(dim=-1)  # [B*T, V, V] 简化处理
            
            attn_out, _ = self.cross_agent_attn[i](
                query=agent_out, key=agent_out, value=agent_out,
                key_padding_mask=attn_mask_cleaned
            )
...
class MapFeatureExtractor(nn.Module):
    def __init__(self, filter_size=128, num_heads=8, max_lanes=64, max_points=30):
        super().__init__()
        self.filter_size = filter_size
...
class RoPE2DSpatialAttention(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int,
                 rope_base: float = 10000.0, init_scene_diam_m: float = 80.0,
...
class ContextEncoder(nn.Module):
    ...
    def __init__(
        ...
        topk_lanes=16,
        max_points=30,
        use_agent_interaction=False,
        use_lane_aware=False,
    ):
        super().__init__()
        self.filter_size = filter_size
        self.topk_lanes = topk_lanes
        self.use_agent_interaction = bool(use_agent_interaction)
        self.use_lane_aware = bool(use_lane_aware)
        ...
        self.map_feature_extractor = MapFeatureExtractor(filter_size=filter_size, num_heads=num_heads, max_points=max_points)
    ...
    def _extract_agent_anchor(self, history_data, history_timestep_mask, history_vehicle_mask, agent_valid_mask):
        """Compute last valid history position per agent for lane selection."""
    ...
    def forward(
        ...
        agent_anchor_xy, agent_anchor_valid = self._extract_agent_anchor(
        ...
        if history_data is not None:
            ...
            # 可选：Agent交互增强
            if self.agent_interaction is not None:
        ...
        if (map_data is not None) and (map_mask is not None):
            ...
            # 修改策略：如果 history_data 缺失（Unconditional/Cold Start），不应该使用 TopK 限制范围，
            # 因为 Agent 可能生成在地图任意位置。此时应提供全量地图。
            # 只有当明确知道 Agent 历史位置时，才使用 TopK 进行局部优化。
            use_topk = (history_data is not None) and (self.topk_lanes > 0)
            
            if use_topk:
                lane_features, lane_pad = self._select_topk_lanes(
                    lane_feat_shared, map_data, map_mask,
                    agent_anchor_xy, agent_valid_mask, agent_anchor_valid
                )
            else:
                # 使用所有车道：扩展 [B, L, F] -> [B, V, L, F] -> [B*V, L, F]
                # 注意：这会增加计算量，但对生成质量至关重要
                lane_features = lane_feat_shared.unsqueeze(1).expand(B, V, L, -1).reshape(B * V, L, self.filter_size)
                
                # 构造 mask: [B, L, P] -> [B, L] -> [B*V, L]
                lane_valid_L = map_mask.any(dim=-1) # 只要有点有效，lane就有效
        ...
        if (history_tokens is not None) and (lane_features is not None):
            ...
            # 可选：Lane-aware attention增强
            if (self.lane_aware_attn is not None) and (history_data is not None) and (map_data is not None):
                # 获取agent位置和速度（使用最后一个有效时间步）
                agent_pos = agent_anchor_xy  # [B, V, 2]
                # 从history_data获取速度
                if history_data.shape[1] >= 4:
                    agent_vel = history_data[:, 2:4, -1, :].permute(0, 2, 1)  # [B, V, 2]
                else:
    ...
    def forward(self, input, condition=None, map_data=None, map_mask=None, agent_types=None, 
        ...
        if self.affine:
            ...
            if timestep_mask is not None:
                ...
                out_b = out_b * mask_b.float() + in_b * (1.0 - mask_b.float())
...
class Block(nn.Module):
    def __init__(
        ...
        self.context_encoder = ContextEncoder(
            ...
            use_agent_interaction=use_agent_interaction,
            use_lane_aware=use_lane_aware,
        )
    ...
    def forward(self, input, condition=None, map_data=None, map_mask=None, agent_types=None, 
                history_data=None, history_timestep_mask=None, target_vehicle_mask=None, history_vehicle_mask=None, timestep_mask=None):
        """
        Args:
    ...
    def reverse(self, output, condition=None, map_data=None, map_mask=None, agent_types=None, eps=None, history_data=None, history_timestep_mask=None, reconstruct=False, target_vehicle_mask=None, history_vehicle_mask=None, timestep_mask=None):
        """
...
class Glow(nn.Module):
    ...
    def forward(self, input, condition=None, map_data=None, map_mask=None, agent_types=None, 
        ...
        for block in self.blocks:
            out, det, log_p, z_new = block(
                out, condition, map_data, map_mask, agent_types, history_data, 
                history_timestep_mask, target_vehicle_mask, history_vehicle_mask, timestep_mask=current_mask
            )
            z_outs.append(z_new)
            logdet = logdet + det
    ...
    def reverse(self, z_list, condition=None, map_data=None, map_mask=None, agent_types=None, history_data=None, history_timestep_mask=None, reconstruct=False, target_vehicle_mask=None, history_vehicle_mask=None, timestep_mask=None):
        """
        Reverse pass for sampling.
        ...
        for i, block in enumerate(self.blocks[::-1]):
            ...
            if i == 0:
                input = block.reverse(
                    z_list[-1], condition, map_data, map_mask, agent_types, z_list[-1], history_data,
                    history_timestep_mask, reconstruct=reconstruct,
                    target_vehicle_mask=target_vehicle_mask, history_vehicle_mask=history_vehicle_mask, timestep_mask=block_mask
                )
            else:
                input = block.reverse(
                    input, condition, map_data, map_mask, agent_types, z_list[-(i + 1)], history_data,
                    history_timestep_mask, reconstruct=reconstruct,
                    target_vehicle_mask=target_vehicle_mask, history_vehicle_mask=history_vehicle_mask, timestep_mask=block_mask
                )
        return input
