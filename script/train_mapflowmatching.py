"""
Training script for MapFlowMatching model using VBD's data pipeline.
Based on script/train_mapglow.py for identical data input and visualization flow.
"""
import sys
import os
...
from vbd.model.MapFlowMatchingWrapper import MapFlowMatchingWrapper
...
class MapFlowMatchingSamplingCallback(pl.Callback):
    def __init__(
        self,
        val_loader,
        output_path,
...
def build_logger(cfg, output_path, model_name):
    ...
    if not use_wandb:
        return CSVLogger(output_path, name="MapFlowMatching", version=1, flush_logs_every_n_steps=100)
    ...
    wandb_kwargs = dict(
        ...
        project=cfg.get("project", "MapFlowMatching"),
        log_model=False,
        dir=output_path,
    )
    if wandb_entity:
...
def train(cfg):
    ...
    print("=" * 60)
    print("Starting MapFlowMatching Training with VBD Pipeline")
    print("=" * 60)
    ...
    model = MapFlowMatchingWrapper(cfg=cfg)
    ...
    if sample_every_n_steps > 0:
        ...
        callbacks.append(
            MapFlowMatchingSamplingCallback(
...
def build_parser():
    ...
    parser = argparse.ArgumentParser(description="MapFlowMatching Training with VBD Pipeline")
    parser.add_argument("-cfg", "--cfg", type=str, default="config/MapFlowMatching.yaml",
